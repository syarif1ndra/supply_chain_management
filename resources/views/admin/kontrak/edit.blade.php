                ...
                <form action="{{ route('admin.kontrak.update', $kontrak->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal_mulai"
                            ...
                            value="{{ $kontrak->tanggal_mulai }}" required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal_selesai"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
                            value="{{ $kontrak->tanggal_selesai }}" required>
                    </div>

                    <div class="mb-4">
                        <label for="deskripsi" class="block text-gray-700 font-bold mb-2">Deskripsi:</label>
                        <textarea name="deskripsi"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>{{ $kontrak->deskripsi }}</textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if ($kontrak->bukti_kontrak)
                            <p class="mt-2"><a href="{{ asset('storage/' . $kontrak->bukti_kontrak) }}" target="_blank"
                                    class="text-blue-500 hover:underline">Lihat Dokumen Saat Ini</a></p>
                        @endif
                    </div>

                    <button type="submit"
                        class="bg-yellow-400 hover:bg-yellow-500 text-dark font-semibold py-2 px-4 rounded shadow-lg transition duration-300 w-full">
                        Simpan Perubahan
                    </button>
                </form>
